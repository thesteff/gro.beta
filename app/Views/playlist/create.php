...
(function($, viewport){
	...
	function add_break() {
		...
		$("#hidden_select").append("<option versionId='-1' selected>-1</option>");
	...
	function move_elem(action) {
		...
		if (action == "to_left" && $(".selected").closest("table").attr("id") == "songlist") {
			...
			new_tr.on("click", function() {
				// On déselectionne la tr précédente
				$(".is_playable tbody").find(".selected").each(function() {
			...
			// On ajoute l'élément au formulaire caché
			$("#hidden_select").append("<option versionId='"+versionId+"' stage='0' selected>"+versionId+" - 0 "+titre+"</option>");
		}
		...
		// Si on veut déplacer à droite et que on a selectionné à gauche		
			...
			versionId = $("#left_list .selected").attr("versionId");
			...
			$("#songlist tr[versionId='"+versionId+"']").css('text-decoration','none');
...
<div class="panel panel-default row">
	...
	<div id="double_list_bar" class="container-fluid" style="padding:15px 0px; margin-left:-15px; margin-right:-15px;">
		...
		<form action="<?php echo base_url(); ?>/playlist/create" method="post">
			...
			<div class="item active col-xs-12 col-sm-5 col-md-5 col-lg-5">
				...
				<div class="" style="display:flex; align-items:center; justify-content:space-between">
					...
					<div style="flex-grow:1">
						...
						<!-- Titre !-->
						<input class="form-control" id="title" type="text" name="title" placeholder="Titre de la playlist" value="" autofocus required/>

						<!-- **** TABLEAU NEW PLAYLIST **** !-->
						...
						<select id="hidden_select" name="song_list[]" multiple style="display:none">
							<?php
								////// On insère dans la hidden_select list la liste des morceaux enregistrés dans le post
								/*foreach ($this->input->post('song_list') as $ref) {
									$versionId = explode(" ",$ref);
									echo "<option versionId='".$versionId[0]."' reserve_stage='".$versionId[3]."' selected>".$versionId[0]." - ".$versionId[3]." ".$versionId[4]."</option>";
								}*/
							?>
						</select>
				...
				<!-- ******* SUBMIT /  BOUTON DE CREATION   ********* -->
				<br />
				<div class="container-fluid text-center">
					<button id="action_btn" class="btn btn-block btn-primary" type="submit" value="submit" name="submit" disabled>Créer la playlist</button>
				</div>
